...
sequence_length = 20
train_df_file_path = 'covid19-global-forecasting-week-4.csv'
extra_data_file_path = 'enriched_covid_19_week_2.csv'
trend_df_file_path = 'trend_df.pkl'
pre_trend_df_file_path = 'pre_trend_df.pkl'
model_file_path = 'model.h5'
pre_model_file_path = 'pre_model.h5'
...
def split_training_validation(trend_df):
    ...
    for idx, row in tmp_df.iterrows():
        if line_number < tmp_df.shape[0] * training_percentage:
...
def train_model(model, X_temporal_train, X_demographic_train, Y_cases_train, X_temporal_test,
    ...
    callbacks = [ReduceLROnPlateau(monitor='val_loss', patience=4, verbose=1, factor=0.6),
                 ...
                 ModelCheckpoint(filepath=os.getcwd() +'\\' + model_file_path, monitor='val_loss', save_best_only=True)]
...
def gen_prediction(model, training_file_path, X_temporal_test, X_demographic_test, validation_df):
    model.load_weights(str(os.getcwd() +'\\' + training_file_path))
...
short_options = "mt"
long_options = ["map_trends", "train_lstm"]
try:
...
if map_trends:
    ...
    trend_df = pd.read_pickle(os.getcwd() +'\\' + pre_trend_df_file_path)
X_temporal_train, X_demographic_train, Y_cases_train, X_temporal_test, X_demographic_test, Y_cases_test,\
validation_df = split_training_validation(trend_df)
model = build_model()
if train_lstm:
    train_model(model, X_temporal_train, X_demographic_train, Y_cases_train, X_temporal_test, X_demographic_test,
                Y_cases_test)
    training_file_path = model_file_path
else:
    training_file_path = pre_model_file_path
gen_prediction(model, training_file_path, X_temporal_test, X_demographic_test, validation_df)
